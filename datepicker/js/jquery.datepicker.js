...
;(function($) {
	...
	$.datePicker = {
		...
		textbox_target = options.element[0].id;
			...
			$(document).on('mouseup', function (e) {
				...
				{
					var tempDate = $.datePicker.utils.pad(parseInt(date.getDate()), 2);
					var tempMonth = $.datePicker.utils.pad(parseInt(date.getMonth()) + 1, 2);					
					
					var thistextbox = $("#" + textbox_target).val().trim();
					var parseToDate = new Date(thistextbox).getTime();
					if (thistextbox != "") {
						if (!parseToDate) {
							$("#" + textbox_target).val(tempDate + "/" + tempMonth + "/" + date.getFullYear());
						}
					}
